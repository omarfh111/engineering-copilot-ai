...
export function TeamCard({ team, onView, onEdit, onDelete }: TeamCardProps) {
  return (
    <article className="page-shell border-white/10 bg-slate-950/60">
      ...
      <p className="mt-4 text-sm leading-7 text-slate-300">{team.description || 'No description provided yet.'}</p>

      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Members</p>
          <p className="mt-2 text-xl font-semibold text-white">{team.memberCount}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Projects</p>
          <p className="mt-2 text-xl font-semibold text-white">{team.projectCount}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Updated</p>
          <p className="mt-2 text-base font-semibold text-white">{formatDate(team.updatedAt)}</p>
        </div>
      </div>
