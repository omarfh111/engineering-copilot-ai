...
export function DocumentsPage() {
  ...
  const handleCreateSubmit = async (payload: CreateDocumentPayload, file?: File) => {
    setSubmitting(true);

    try {
      await adminDocumentService.createDocument(payload, file);
      showToast({
        type: 'success',
        title: 'Document created',
        description: file ? `${payload.title} was uploaded and sent for indexing.` : `${payload.title} was linked successfully.`
      });
      setCreateOpen(false);
      await loadDocuments();
    } catch (createError) {
        ...
        <>
          <DocumentFormModal
            ...
            onSubmit={(payload, file) => {
              void handleCreateSubmit(payload as CreateDocumentPayload, file);
            }}
            open={createOpen}
            projects={projects}
          ...
          <DocumentFormModal
            ...
            onSubmit={(payload) => {
              void handleEditSubmit(payload as UpdateDocumentPayload);
            }}
            open={editOpen}
            projects={projects}
