...
type ActionFilter = 'ALL' | 'POST' | 'PUT' | 'PATCH' | 'DELETE';
...
function exportPdf(logs: AuditLogEntry[]) {
  ...
  const content = lines
    .map((line, index) => `BT /F1 9 Tf 40 ${760 - index * 15} Td (${escapePdfText(line.slice(0, 120))}) Tj ET`)
    .join('\n');
  const objects = [
...
export function AuditLogsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setTotalEvents(response.totalElements ?? 0);
  ...
  const handleExportCsv = async () => {
    downloadBlob(new Blob([buildCsv(logs)], { type: 'text/csv;charset=utf-8' }), 'engineering-copilot-audit.csv');
    ...
    <div className="space-y-6">
      <section className="page-shell bg-gradient-to-br from-brand-900 via-brand-700 to-brand-500 text-white">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-brand-50/85">
          Chronological governance trail for identity, security, and platform operations.
        </p>
      </section>
      ...
      <section className="page-shell space-y-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              className="inline-flex items-center gap-2 rounded-2xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-brand-500/20 transition hover:bg-brand-600 disabled:opacity-60"
            ...
            <button
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand-300 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
              disabled={logs.length === 0}
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,2fr)_180px_180px_auto]">
          ...
          <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
            ...
            <select
              ...
              value={statusFilter}
              onChange={(event) => {
                setStatusFilter(event.target.value as 'ALL' | AuditLogStatus);
                setPage(0);
              }}
            >
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-900">
              <tr>
                {['Timestamp', 'Actor Email', 'Action', 'IP Address', 'Status'].map((heading) => (
                  <th className="px-5 py-4 text-left text-xs font-bold uppercase tracking-[0.18em] text-slate-500" key={heading}>
                    {heading}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-950">
              {loading ? (
                ...
                <tr>
                  <td className="px-5 py-8 text-center text-sm text-rose-600 dark:text-rose-300" colSpan={5}>
                    {error}
                  </td>
                </tr>
              ) : logs.length === 0 ? (
        ...
        <div className="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400">
          ...
          <div className="flex gap-2">
            <button
              className="rounded-xl border border-slate-200 px-3 py-2 font-semibold disabled:opacity-40 dark:border-slate-700"
