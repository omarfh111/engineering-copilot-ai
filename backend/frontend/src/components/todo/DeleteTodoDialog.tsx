...
export function DeleteTodoDialog({ open, todo, loading, onCancel, onConfirm }: DeleteTodoDialogProps) {
  if (!open || !todo) {
    return null;
  }
      ...
      <div className="animate-slideUp w-full max-w-lg rounded-[32px] border border-white/60 bg-white p-6 shadow-panel dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Delete todo</h3>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">This permanently removes the selected remediation item.</p>
            </div>
          </div>
          <button
