...
export function DeleteAnalysisDialog({ open, analysis, loading, onCancel, onConfirm }: DeleteAnalysisDialogProps) {
  if (!open || !analysis) {
    return null;
  }
      ...
      <div className="animate-slideUp w-full max-w-lg rounded-[32px] border border-white/60 bg-white p-6 shadow-panel dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Delete analysis</h3>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                This permanently removes the selected analysis record.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="mt-5 rounded-2xl border border-rose-200 bg-rose-50/80 p-4 text-sm text-rose-800 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-200">
          You are about to delete <strong>{analysis.title}</strong>.
        </div>
