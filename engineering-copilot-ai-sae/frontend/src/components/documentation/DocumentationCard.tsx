    ...
    <article className="page-shell border-white/10 bg-slate-950/60">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white">{documentation.title}</h3>
            <p className="text-sm text-slate-400">{formatDocumentationType(documentation.type)}</p>
          </div>
        </div>
      ...
      <p className="mt-4 text-sm leading-7 text-slate-300">{documentation.description || 'No description provided yet.'}</p>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          ...
          <div className="mt-2">
            <StatusBadge
              label={formatDocumentationStatus(documentation.status)}
              tone={getDocumentationStatusTone(documentation.status)}
            />
          </div>
        </div>
        {showProjectMeta ? (
          <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
            ...
            <p className="mt-2 text-base font-semibold text-white">{documentation.project.title}</p>
          </div>
        ) : null}
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Generated externally</p>
          <p className="mt-2 text-base font-semibold text-white">{documentation.generatedByAI ? 'Yes' : 'No'}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Approved</p>
          <p className="mt-2 text-base font-semibold text-white">{documentation.approved ? 'Yes' : 'No'}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Created</p>
          <p className="mt-2 text-base font-semibold text-white">{formatDate(documentation.createdAt)}</p>
        </div>
      </div>
      ...
      {documentation.path ? (
        <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3">
          ...
          <p className="mt-2 break-all text-sm font-semibold text-white">{documentation.path}</p>
        </div>
      ) : null}
